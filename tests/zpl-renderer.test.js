...
test('gera uma etiqueta por produto', () => {
  const partes = renderizarEtiquetaCaixaZpl(documento);
  ...
  assert.match(partes[0].payload_zpl, /\^PW799/);
  assert.match(partes[0].payload_zpl, /\^LL559/);
});

test('preenche campos do produto na faixa topo', () => {
  const [p1] = renderizarEtiquetaCaixaZpl(documento);
  assert.match(p1.payload_zpl, /\^FDIT1\^FS/);
  assert.match(p1.payload_zpl, /\^FDOP1\^FS/);
});

test('preenche campos operacionais na faixa meio', () => {
  const [p1] = renderizarEtiquetaCaixaZpl(documento);
...
test('inclui sequencia da caixa quando presente', () => {
  const [p1] = renderizarEtiquetaCaixaZpl(documento);
  assert.match(p1.payload_zpl, /\^FD\(2\/5\)\^FS/);
});

test('QR code contém JSON estruturado', () => {
  const [p1] = renderizarEtiquetaCaixaZpl(documento);
  assert.match(p1.payload_zpl, /\^BQN,2,5/);
  assert.match(p1.payload_zpl, /\^FDMA,\{"e":"E1","cx":"CX1","op":"OP1","qt":15,"seq":"2\/5"\}/);
});
...
test('NF vazia quando ausente', () => {
  ...
  const [p1] = renderizarEtiquetaCaixaZpl(doc);
  // Valor NF deve estar vazio
  assert.match(p1.payload_zpl, /N\.F:/);
  assert.match(p1.payload_zpl, /\^FD\^FS/);
});

test('mostra paginação quando múltiplas partes', () => {
  const partes = renderizarEtiquetaCaixaZpl(documento);
  assert.match(partes[0].payload_zpl, /PARTE 1\/2/);
  assert.match(partes[1].payload_zpl, /PARTE 2\/2/);
});

test('omite paginação com parte única', () => {
  const doc = { ...documento, linhas: [documento.linhas[0]] };
  const [p1] = renderizarEtiquetaCaixaZpl(doc);
  assert.doesNotMatch(p1.payload_zpl, /PARTE/);
});
...
test('respeita dimensões customizadas via config', () => {
  const [p1] = renderizarEtiquetaCaixaZpl(documento, { larguraDots: 1181, alturaDots: 709 });
  assert.match(p1.payload_zpl, /\^PW1181/);
  assert.match(p1.payload_zpl, /\^LL709/);
});
